...
import numpy as np
import pyblp
log = open("RegressionResults/plain_candy_gum_nestedshare_1.log", "w")
sys.stdout = log
...
def GenerateDEData(products, quarterOrMonth, inputs):
    ...
    # Plain Logit
    logit_formulation = pyblp.Formulation('0 + prices + nestedshare', absorb = 'C(product_ids) + C(city_ids)')
