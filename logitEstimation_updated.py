...
def GenerateDEData(products, quarterOrMonth, inputs, characteristics):
    ...
    variables = ['dma_code_' + quarterOrMonth,'log_adjusted_price','upc','market_share','distance','time'] + products + inputs + characteristics
    print(variables)
    ...
    # Plain Logit
    logit_formulation = pyblp.Formulation('0 + prices + mint + chocolate + CANDY + GUM', absorb = 'C(upc_dma_code) + C(time)')
    ...
    nl_formulation = pyblp.Formulation('0 + prices + mint + chocolate + CANDY + GUM', absorb = 'C(upc_dma_code) + C(time)')
