...
def get_corr_matrix(code, years, groups, modules, merger_date, upcCutoff = '0.96', pre_months = 24, post_months = 24, brand_number = 10):
    ...
    for group, module in zip(groups, modules):
        ...
        if len(upcs_list) > 0:
            ...
            print(upcs_df)
            min_upcs = upcs_df.groupby(['upc_1','upc_2','brand_code'], as_index = False).agg({'year': 'last', 'group': 'first', 'module': 'first', 'correlation': 'min'})
            corr_upcs = min_upcs[min_upcs['correlation'] > float(upcCutoff)]
            corr_upcs_list.append(corr_upcs)

    corr_df = pd.concat(corr_upcs_list)
    corr_df = corr_df.groupby(['upc_1','upc_2','brand_code','group','module'], as_index = False).agg({'correlation': 'min'})
    print(corr_df)
    corr_df.to_csv('../../../All/m_' + code + '/intermediate/correlated_upcs_' + upcCutoff + '_cutoff.csv', sep = ',', index = False)

code = sys.argv[1]
upcCutoff = sys.argv[2]
...
get_corr_matrix(code, years, groups, modules, merger_date, upcCutoff)
