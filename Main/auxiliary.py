...
def append_owners(code, df, month_or_quarter):
	...
	if month_or_quarter == 'month':
		...
		sqlcode = '''
		select df.upc, df.year, df.month, df.prices, df.shares, df.dma, df.brand_code_uc, brand_to_owner.owner
		from df
		...
		sqlcode = '''
		select df.upc, df.year, df.quarter, df.prices, df.shares, df.dma, df.brand_code_uc, brand_to_owner.owner
		from df
		inner join brand_to_owner on df.brand_code_uc=brand_to_owner.brand_code_uc AND df.date >= brand_to_owner.start_date AND df.date <= brand_to_owner.end_date
		'''
	print(df.columns)
