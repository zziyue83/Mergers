...
def sample_demographics(year,ids,ndraw):

        # First, pull PUMA shares by DMA for the given year
        dma_to_puma = pull_dma_shares(year)

        # Next, pull the PUMS data for that year
        pums_path = '../../Data/Demographics/pums_' + str(year) + '.csv'
        pums_data = pd.read_csv(pums_path, delimiter = ',',dtype=str)
        pums_data[ids] = pums_data[ids].astype('float64')
        ...
        for dd in dma_unique:
                ...
                rel_vars = ['YEAR','dma_code','weight'] + ids
                if count == 1:
...
ndraw_str = sys.argv[3].split(',')
ndraw = [int(nn) for nn in ndraw_str]

for yr in years:
        sample_demographics(yr,ids,ndraw)
