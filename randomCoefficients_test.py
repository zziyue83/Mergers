...
log = open("rc_logit_sample_10_iterations.log", "a")
sys.stdout = log
...
def GenerateDEData(products, quarterOrMonth, inputs, characteristics):
    ...
    #resultDf1.to_csv('RegressionResults/test_' + '_'.join([str(elem) for elem in products]) + '_' + quarterOrMonth + '_rc_logit_grid_ones_sample_10_iterations.csv', sep = ',')
    results2 = grid_problem.solve(sigma=np.eye(dim), optimization=bfgs, method='1s')
    print(results2)
    resultDf2 = pd.DataFrame.from_dict(data=results2.to_dict(), orient='index')
    resultDf2.to_csv('RegressionResults/test_' + '_'.join([str(elem) for elem in products]) + '_' + quarterOrMonth + '_rc_logit_grid_diagonal_sample_10_iterations.csv', sep = ',')
