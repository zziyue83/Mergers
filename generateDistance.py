...
def geocoding_distribution_centers(product):
    distribution_centers = pd.read_excel(product + " Distribution Centers.xlsx")
    distribution_centers1 = distribution_centers.dropna(subset=['Location'])
    cities = distribution_centers1['Location'].tolist()
    geolocator = Nominatim()
    geocoding_distribution_centers = pd.DataFrame()
    i = 0
    while i < len(cities):
        try:
            location = geolocator.geocode(cities[i])
            latitude, longitude = location[1]
            a_row = pd.Series([cities[i], latitude, longitude])
            row_geocoding_distribution_centers = pd.DataFrame([a_row])
            geocoding_distribution_centers = pd.concat([row_geocoding_distribution_centers, geocoding_distribution_centers])
            i = i + 1
            print(i)
        except:
            time.sleep(10)
    geocoding_distribution_centers = geocoding_distribution_centers.rename(columns = {0: 'Location', 1: 'distribution_centers_latitude', 2: 'distribution_centers_longitude'})
    geocoding_distribution_centers = geocoding_distribution_centers.set_index('Location')
...
def generateDistance(products, quarterOrMonth):
    ...
    for product in products:
        distribution_centers_product = geocoding_distribution_centers(product)
        distribution_centers = pd.concat([distribution_centers, distribution_centers_product])
    distribution_centers = distribution_centers[['Firm','distribution_centers_latitude','distribution_centers_longitude']]
    ...
    data = data.merge(distribution_centers, left_on = 'owner initial', right_on = 'Firm', how = 'outer')
