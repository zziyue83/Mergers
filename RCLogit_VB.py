...
def SampleRCLogit(product, frequency, inputs, characteristics, start, end, demographics=False):
    try:
        ...
        bfgs = pyblp.Optimization('l-bfgs-b', {'gtol': 1e-10})

        with pyblp.parallel(12):
            results = grid_problem.solve(sigma=np.eye(3), optimization=bfgs, method='1s', iteration = pyblp.Iteration('anderson', method_options = {'fatol': 1e-14, 'maxiter' : 5000}), shares_bounds = (1e-100, None))
        print(results)
        resultDf = pd.DataFrame.from_dict(data=results.to_dict(), orient='index')
