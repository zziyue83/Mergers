...
def GenerateDEData(products, quarterOrMonth, inputs, characteristics):
    ...
    resultDf1.to_csv('RegressionResults/test_' + '_'.join([str(elem) for elem in products]) + '_' + quarterOrMonth + '_rc_logit_grid_ones.csv', sep = ',')
    ...
    resultDf2.to_csv('RegressionResults/test_' + '_'.join([str(elem) for elem in products]) + '_' + quarterOrMonth + '_rc_logit_grid_diagonal.csv', sep = ',')
