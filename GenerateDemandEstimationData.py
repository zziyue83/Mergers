...
def GenerateDEData(product, frequency, inputs, characteristics, start, end):
    ...
    # u = np.linalg.inv(y)
    # print(u)
    variables = ['dma_code_'+frequency,'log_adjusted_price','upc','market_share','distance','y-m'] + characteristics + inputs
    print(variables)
    ...
    logit_formulation = pyblp.Formulation('prices', absorb='C(product_ids) + C(y-m)')
